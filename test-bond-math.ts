...
async function run() {
     ...
     if (!isMatured) {
        ...
        symLots.forEach(lot => {
           ...
           if (instrument?.coupon_rate === 0 || instrument?.coupon_rate == null) {
              const val = calculateTBillConsideration(marketPrice, lot.units);
              bondValue += val;
              console.log(`TBILL ${sym}: units=${lot.units}, marketPrice=${marketPrice}, value=${val}`);
           } else {
              const val = calculateTotalConsideration(marketPrice + accruedCoupon, lot.units, faceValueMultiplier);
              bondValue += val;
              console.log(`BOND ${sym}: units=${lot.units}, marketPrice=${marketPrice}, accrued=${accruedCoupon}, faceValMult=${faceValueMultiplier}, value=${val}`);
           }
        });
     }
  });
  ...
  console.log(`New Bond Value: ${bondValue}`);
}
